...
team.data <- mutate(team.data, Asstpg = AST/G) %>%
  arrange(-Asstpg)
...
good.defenese <- filter(team.data, BLK > 470)
...
defense.stats <- select(team.data, DRB, STL, BLK)
...
BetterShootsers <- function(team1, team2) {
  better.team <- filter(Team %>% c(team1, team2)) %>%
    filter(FG == max(FG.)) %>%
    select(TEAM, FG., PTS)
  return(better.team)
}
